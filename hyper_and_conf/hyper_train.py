...
class Padded_Accuracy(Mean):
    def __init__(self, pad_id=0):
        super(Padded_Accuracy, self).__init__(name='Padded_Accuracy')
        self.pad_id = pad_id
...
class Padded_Seq_Accuracy(Mean):
    def __init__(self, k=10, pad_id=0):
        super(Padded_Seq_Accuracy, self).__init__(name='Padded_Seq_Accuracy')
        self.pad_id = pad_id

    def update_state(self, y_true, y_pred, sample_weight=None):
        y_true = math_ops.cast(y_true, self._dtype)
        y_pred = math_ops.cast(y_pred, self._dtype)
        self.score = conf_metrics.padded_sequence_accuracy_score(
            y_true, y_pred)
        return super(Padded_Seq_Accuracy, self).update_state(
...
class Padded_Accuracy_topk(Mean):
    def __init__(self, k=10, pad_id=0):
        super(Padded_Accuracy_topk, self).__init__(name='Padded_Accuracy_topk')
        self.k = k
        self.pad_id = pad_id
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        y_true = math_ops.cast(y_true, self._dtype)
        y_pred = math_ops.cast(y_pred, self._dtype)
        self.score = conf_metrics.padded_accuracy_score_topk(y_true, y_pred)
...
class Approx_WER_Metrics(Mean):
    def __init__(self, eos_id=1):
        super(Approx_WER_Metrics, self).__init__(name='Approx_WER')
        self.eos_id = eos_id
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        if y_length is not None:
            ...
            y_pred = tf.cast(tf.argmax(input=y_pred, axis=-1), dtype=tf.int32)

        wer = conf_metrics.wer_score(y_pred, y_true, self.eos_id)
        return super(Approx_WER_Metrics, self).update_state(
            wer, sample_weight=sample_weight)
...
class Approx_BLEU_Metrics(Mean):
    def __init__(self, eos_id=1):
        super(Approx_BLEU_Metrics, self).__init__(name='Approx_BLEU')
        self.eos_id = eos_id
...
class Learning_Rate_Reporter(Metric):
    def __init__(self):
        super(Learning_Rate_Reporter, self).__init__(dtype=tf.float32)
...
class StagingAreaCallback(Callback):
    ...
    def __init__(self, x, y, batch_size, prefetch_count=1):
        ...
        with tf.device('/cpu:0'):
            ...
            features_batch_next_value_src = tf.compat.v1.placeholder(
                dtype=x[0].dtype, shape=features_shape_src)
            features_batch_next_value_tgt = tf.compat.v1.placeholder(
            ...
            self.labels_batch_next_value = tf.compat.v1.placeholder(
