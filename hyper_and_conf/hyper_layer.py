...
class Positional_Encoding(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        position = tf.cast(tf.range(inputs), dtype=tf.float32)
    ...
    def get_config(self):
        config = super(Positional_Encoding, self).get_config()
        c = {
...
class Multi_Head_Attention(tf.keras.layers.Layer):
    ...
    def call(self, inputs, K_V=None, bias=0, cache=None, training=False):
        ...
        # K = self.ln(K)
        # V = self.ln(V)
        with tf.compat.v1.name_scope('Q_K_V_linear_projection'):
        ...
        with tf.compat.v1.name_scope('attention_linear_projection'):
    ...
    def scale_dot_product_attention(self, Q, K, V, bias, training=False):
        with tf.compat.v1.name_scope('scale_dot_product'):
            Q = self.split_heads(Q)
            K = self.split_heads(K)
            V = self.split_heads(V)
            d_Q = tf.cast(tf.shape(input=Q)[-1], tf.float32)
    ...
    def combine_heads(self, x):
        with tf.compat.v1.name_scope("combine_heads"):
            ...
            x = tf.transpose(
                a=x, perm=[0, 2, 1,
                           3])  # --> [batch, length, num_heads, depth]
